...
public class PssRestController {
    ...
    //-------
    @PostMapping("/changeDelegationProp/delegationId={delegationId}")
    public boolean deleteDelegation(@PathVariable Long delegationId, @RequestParam("accepted") boolean accepted, @RequestParam("pending") boolean pending, @RequestParam("cancel") boolean cancel){
        ...
        {
            delegation.get().setDelegationAccepted(accepted);
            delegation.get().setDelegationPending(pending);
            delegation.get().setDelegationRequestCancel(cancel);
